...
sys.path.append(os.path.abspath('SkinGPT-4'))
...
def main():
    ...
    config = {
        'model': {
            ...
            'max_txt_len': 160,
            'end_sym': "###",
            'prompt_path': "SkinGPT-4/prompts/alignment_skin.txt",
            'prompt_template': '###Human: {} ###Assistant: ',
            'ckpt': "SkinGPT-4/weights/skingpt4_llama2_13bchat_finetune_v4.2_small_class.pth",
    ...
    config_path = "SkinGPT-4/eval_configs/skingpt4_eval_llama2_13bchat_local.yaml"
    ...
    ham_df.to_csv("results/ham10000_results.csv", index=False)
    print(f"Saved HAM10000 results to results/ham10000_results.csv")
    
    # Evaluate SCIN dataset
    print("\nEvaluating SCIN dataset...")
    scin_results = evaluator.evaluate_dataset(
        "data/scin/images",
        "data/scin/scin_merged.csv",
        num_samples=100  # Start with a small sample for testing
    )
    
    # Save SCIN results
    scin_df = pd.DataFrame(scin_results)
    scin_df.to_csv("results/scin_results.csv", index=False)
    print(f"Saved SCIN results to results/scin_results.csv")
